...
async def analyze():
    ...
    with_context = await db.inbound_messages.count_documents({"raw_payload.context.id": {"$exists": True}})
    ...
    cursor = db.inbound_messages.find({"raw_payload.context.id": {"$exists": False}}).sort("received_at", -1).limit(100)
    ...
    async for msg in cursor:
        ...
        if phone and dt:
            # find last message_log sent to this phone before this message
    ...
    print(f"Out of 100 recent inbound messages without context.id, {found_campaign} had a recent outbound campaign message to that phone.")
