...
if (!fs.existsSync(path.join(__dirname, '..', '.venv'))) {
  ...
  const venvCreate = spawnSync(pythonCmd, ['-m', 'venv', '.venv'], { stdio: 'inherit', shell: false, cwd: backendDir });
  if (venvCreate.status !== 0) {
    console.error('Failed to create virtual environment. Ensure python is installed and on your PATH.');
...
const pipInstall = spawnSync(pipBin, ['install', '-r', 'requirements.txt'], { stdio: 'inherit', shell: false, cwd: backendDir });
