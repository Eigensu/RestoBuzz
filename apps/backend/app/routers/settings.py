...
async def list_suppression(
    ...
    cursor = db.suppression_list.find({}).sort("added_at", -1).skip(skip).limit(page_size)
    items = []
    async for doc in cursor:
        items.append(SuppressionResponse(
            id=str(doc["_id"]),
            phone=doc["phone"],
            reason=doc["reason"],
...
async def add_suppression(
    ...
    try:
        ...
        raise HTTPException(409, "Phone already in suppression list")

    return SuppressionResponse(id=str(result.inserted_id), **{k: v for k, v in doc.items() if k != "_id"},
...
async def remove_suppression(
    ...
    if result.deleted_count == 0:
        raise HTTPException(404, "Phone not found in suppression list")
